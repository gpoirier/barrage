...
class BoardStateSpec extends AnyFlatSpec with Matchers {
  ...
  it should "overflow dams correctly" in {
    ...
    val basin2 = DamBasin(
      upper = DamSpot(height = 0, water = 0),
      lower = DamSpot(height = 2, water = 1),
      overflowsTo = None
    )
    //this is the upper basin, but I get a "forward reference" compile error if I declare it above
    val basin1 = DamBasin(
      upper = DamSpot(height = 1, water = 1),
      lower = DamSpot(height = 3, water = 0),
    ...
    after.basins shouldBe Set(
      DamBasin(
        upper = DamSpot(height = 1, water = 1),
        lower = DamSpot(height = 3, water = 3),
        overflowsTo = Some(basin2)
      ),
      DamBasin(
        upper = DamSpot(height = 0, water = 0),
        lower = DamSpot(height = 2, water = 2),
        overflowsTo = None
      )
    )
    ...
    after2.basins shouldBe Set(
      ...
      DamBasin(
        upper = DamSpot(height = 1, water = 1),
        lower = DamSpot(height = 3, water = 1),
        overflowsTo = Some(basin2)
      )
    )
